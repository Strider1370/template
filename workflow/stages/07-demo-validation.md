...
3. **예비 검증 = 서브에이전트.** 동시에 데모 검증 서브에이전트를 띄운다. **목적은 "작동하나?" 재확인이 아니라(그건 1번에서 사람이 끝냄) — 라이브 실패 대비 백업 캡처 + 재현성 + manifest 정직성 대조.** `demo.scenario.yaml`대로 핵심경로 **2회 완주** + **단계별 스크린샷 + Wow 캡처 + 짧은 리포트**. (AI가 preview 도구로 "되나?"를 장시간 루프 도는 것 금지 — 사람 1초 확인이 더 빠르고 확실하다.)
...
- **모바일 산출물 시연(사용자 결정)**: **기본 = 네이티브 APK를 폰에 설치하고 USB+scrcpy로 미러링**(서버 https 앱 로드). USB 안 잡히면 **무선 페어링 폴백**, 그래도 안 되면 PWA(홈 화면 추가). 불안정하면 단계별 스크린샷 백업. **절차·명령은 `docs/deploy-runbook.md` C3(USB)/C4(무선).** APK 빌드는 서버에서(C2).
