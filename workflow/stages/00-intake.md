...
## 6. 메인 에이전트의 역할
- **(맨 먼저 — 비차단) 환경세팅 서브에이전트를 백그라운드로 띄운다.** 계약 = `workflow/templates/env-setup-task.yaml` (배포+시연 툴체인 점검/설치, 사람 할 일 체크리스트 산출). 띄운 직후 사용자에게 "환경 점검이 뒤에서 도는 중"이라 알리고, **메인은 대기하지 말고 아래 intake/발산을 이어간다.** 완료 알림이 오면 결과(GO/사람 할 일)를 사용자에게 요약 보고한다. (새 환경이면 특히 유용 — 느린 설치/점검을 발산과 겹쳐 벽시계 시간을 줄인다.)
...
## 7. 병렬 서브에이전트 구성
- **환경세팅(env-setup) 1개를 백그라운드로** — 계약 `workflow/templates/env-setup-task.yaml`. 보고서 → `workflow/history/stage-00-env-preflight.md`. (그 외 intake 기록은 메인이 직접.)
