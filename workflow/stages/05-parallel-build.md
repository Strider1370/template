...
- AI: AI 호출 모듈 경로 — **재사용 헬퍼 `web/lib/llm.ts`(`openaiChat`: OpenAI 기본·모델 자동분기·키 없으면 null) + `/api/llm` 라우트를 그대로 쓴다(같은 것 새로 만들지 마라)**. **키는 `.env`가 아니라 앱 런타임 입력(설정/더보기 ⚙️·⋯ 아이콘 → 키 모달, 메모리/sessionStorage 저장) → 요청 헤더(`x-llm-key`)로 실어 `/api/llm`이 그 키 사용**(표준: `workflow/decisions/llm-key-input.md`). 완료: **설정 아이콘+키 모달 구현** + 폴백(하드코딩 응답)으로 **키 없을 때도 완전히 동작** + 키 절대 커밋 금지.
