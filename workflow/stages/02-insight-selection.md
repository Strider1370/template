...
1. **(a) 발산 — 사용자와 주거니받거니 (최소 2~3라운드).** Insight·아이디어 후보를 ≥5개 던지되, **초보자에게 설명하듯** 각 후보가 무슨 뜻이고 왜 흥미로운지 한두 줄 곁들인다. 사용자의 반응·반박·"이 방향 더 파봐"를 받아 다시 던지는 식으로 **여러 라운드** 넓힌다. **한 번에 수렴하거나 곧장 (d) 승인으로 건너뛰지 마라** — 사용자가 "이제 좁히자"고 하기 전까지 발산을 이어간다.
...
4. **(d) 사용자 승인 🙋.** `AskUserQuestion`으로 방향을 제시하고 승인을 받는다. **제시할 때 초보자에게 설명하듯** 각 방향이 무슨 뜻인지·왜 좋은지(킥/AI 필요성)·예상되는 데모 모습을 쉬운 말로 풀고, 추천안이 있으면 이유를 댄다(결론만 던지지 말 것). 승인 전엔 멈춘다(`awaiting_approval`).
...
## 18. 금지 사항
- **(a) 발산 없이 곧장 승인으로 직행 금지** — 최소 2~3라운드 주고받기 후에야 (d)로 간다.
