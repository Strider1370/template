...
| OpenAI API 키 (`OPENAI_API_KEY`) | 앱이 런타임에 LLM을 호출할 때(자연어 파싱·설명 rewrite·되묻기). 기본 모델 `gpt-4.1-mini`, `OPENAI_MODEL`로 교체. 없으면 폴백으로 동작 |
...
| 공공데이터 | 전국 행정경계 · 민방위 대피소 약 17,000곳 · 지도/지역 데이터 (상세는 아래 6.1) |
...
대회 전에 공개 데이터를 미리 준비하는 것은 허용됩니다(반입이 금지되는 건 "완성형 결과물"뿐입니다). 그래서 자주 쓰는 공공데이터를 두 형태로 갖춰 뒀습니다. 어떤 데이터가 준비돼 있는지는 아래로 확인하면 됩니다.

**① 바로 쓸 수 있게 번들된 데이터** — 인증 없이 저장소에 포함되어 있습니다.

| 데이터 | 위치 | 내용 |
|---|---|---|
| 전국 행정경계 | `data/boundaries/` | 시도 경계(GeoJSON·TopoJSON) + 시군구 경계(TopoJSON) |
| 민방위 대피소 | `web/public/data/shelters/civildefense/` | 전국 17개 시도 · 약 17,000곳 (지역별 JSON + `_index.json`) |
| 시도→시군구 매핑 | `web/lib/regions.ts` | 지역 선택 UI의 출발점 |
| 지도 | `web/components/KakaoMap.tsx` | 카카오맵 컴포넌트(키만 넣으면 동작) |

**② 당일 확보용 가이드** — 출처와 표준 스키마를 미리 정리해 두었습니다. 데이터 자체는 대회 때 받습니다.

| 가이드 | 다루는 데이터 |
|---|---|
| `data/data-sources.md` | 복지·혜택(복지로·보조금24), 생활·안전(무더위/한파쉼터·지진대피·CCTV·안전비상벨 등), 지도 키 — 출처·신청 페이지 목록 |
| `docs/welfare-benefit-dataset-prep.md` | 복지·혜택 집중 — 혜택 API·KOSIS 통계·시설 POI·정책 근거 + 표준 스키마(`Benefit`/`EvidenceStat`/`Facility`) + `SOURCE_LOG` 템플릿 |

②는 대회 전에 데이터를 받아 두거나 출처·키만 준비해 두면 됩니다. 키는 `.env.local`에만 두고 커밋하지 않습니다.
